...
(function($, undefined) {
	...
	Tree.prototype = {
													...
													if (value.type === "folder") {
														...
														if ("additionalParameters" in value
															...
															$entity
																	.find("i")
																	.removeClass(
																			self.options["unselected-icon"])
																	.addClass(
																			self.options["selected-icon"])
														}
													}
		...
		selectFolder : function(el) {
			...
			var target = "."
					+ $.trim(this.options["close-icon"].replace(/\s/g, "."));
			...
			if ($el.find(target).length) {
				...
				target = "."
						+ $.trim(this.options["open-icon"].replace(/\s/g, "."));
				eventType = "closed";
			...
			$parent.find(target).eq(0).removeClass(classToTarget).addClass(
					classToAdd);
